//Basic and minimalistic linear regression for dense data, SGD
...
void UpdateGradsOneInstance(int d){
	for(int g = 0; g < gradients.size(); g++)
		gradients[g] = 2*(predictions[d] - labels[d])*data[d][g];	
}
...
void LinearRegression(int passes, int features, enum Setup setup, double learning_rate){
...
int main(int argc, char *argv[]){
	...
	cin >> learning_rate;
	LinearRegression(passes, features, setup, learning_rate);	
	PrintWeights();
	return 0;
}
